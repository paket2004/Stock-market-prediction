...
def test_read_datastore():
    """Test the read_datastore function with mocked dependencies."""

    # Mock data
    mock_data_path = '/mocked/path/data/samples/sample.csv'
    mock_df = pd.DataFrame({'column1': [1, 2, 3], 'column2': ['a', 'b', 'c']})

    # Mock dvc.api.get_url to return the mock data path
    with mock.patch('dvc.api.get_url', return_value=mock_data_path):
